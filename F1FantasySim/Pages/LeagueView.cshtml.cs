    ...
    public class LeagueViewModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            ...
            foreach (var member in apiModel.memRank)
            {
                ...
                handler = GetHttpClientWithCookies(memberUri, playerCookies);
                using (var client = new HttpClient(handler))
                {
                    client.DefaultRequestHeaders.Add("User-Agent", UserAgent);

                    var playerDetails = await GetApiData<PlayerDetails>(memberUri, client);
        ...
        private async Task<T> GetApiData<T>(string url, HttpClient client = null)
        {
            client ??= _httpClient;
